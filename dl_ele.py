...
for i in base:
    ...
    for x in d_epid:
        ...
        for z in my:
            ...
            my_dtime = z[3]
            if dump>my_ele:
                sql_cz = '''SELECT top 1 epid,xnum,ele,dtime dtime FROM kddz.dbo.sellelerecords WHERE epid='%s' AND xnum='%s' ORDER BY dtime DESC '''
                ...
                for x in cz:
                    ...
                    if pd_sy ==():
                        ...
                        for c in w_phone:
                            ...
                            ins_amm = '''INSERT INTO test.dl_ammeter_user(dl_uuid,dl_status_date,phone) VALUES('%s','%s','%s')'''
                            amm = my_insert(ins_amm,(uuid,cz_dtime,phone))
                            if len(phone) == 0:
